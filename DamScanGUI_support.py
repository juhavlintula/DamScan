...
#    Sep 27, 2017 09:48:07 PM

# version history:
# 0.0.5 Ini-file for DamScan.py with ConfigParser
# 0.0.6 Selectable Ini-files for DamSacn,
#       ini-file for GUI also based on ConfigParser (hardcoded name "DAMScanGUI.config" in current directory)
# 0.0.7 Cleaner GUI panel and enabled Tab-navigate between text fields on the same line
# 0.0.8 Tabbed Notebook for Scan or Compare
# 0.0.9 Support for DamCompare.py verson 1.3.0, no import of DamScan/DamCompare necessary
# 0.0.10 Browse INI files, DAMScanGUIcon.ini changed to DAMScanGUI.config
# 0.0.12 Browse for Daminion Catalogs, configuration & output file and edit files via buttons
# 0.0.13 Menu & keyboard shortcuts
# 0.0.14 Switch tabs based on radio button & select option sets (ini-file) from scrollable list
...
# 0.1.3a Should not crash, when server or port are wrong
...
def set_Tk_var():
    global txt_dbpath
    txt_dbpath = StringVar()
    global txt_dbname
    txt_dbname = StringVar()
    global txt_user
    txt_user = StringVar()
    global txt_password
    ...
    txt_port = StringVar()
    global ckb_sqlite
    ckb_sqlite = StringVar()
    global txt_catalog_dates
    txt_catalog_dates = StringVar()
    global txt_catalog2_dates
    txt_catalog2_dates = StringVar()
    global lst_netcatalogs
    lst_netcatalogs = StringVar()
    global rbtn_verbose
    rbtn_verbose = StringVar()
    global ckb_display_only
    ckb_display_only = StringVar()
    global ckb_itemid
    ckb_itemid = StringVar()
    global ckb_fullpath
    ckb_fullpath = StringVar()
    global txt_outfilename
    txt_outfilename = StringVar()
    global lstb_recent_ini_files
    lstb_recent_ini_files = StringVar()
    global txt_newdamscanini
    txt_newdamscanini = StringVar()
    global txt_damscanini
    txt_damscanini = StringVar()
    global txt_basename
    txt_basename = StringVar()
    global txt_exfile
    txt_exfile = StringVar()
    global txt_onlyfile
    txt_onlyfile = StringVar()
    global txt_ack_pairs
    txt_ack_pairs = StringVar()
    global txt_pathconfigfiles
    txt_pathconfigfiles = StringVar()
    global txt_damscanpgm
    txt_damscanpgm = StringVar()
    global ckb_group
    ckb_group = StringVar()
    global txt_dbpath2
    txt_dbpath2 = StringVar()
    global txt_dbname2
    txt_dbname2 = StringVar()
    global txt_damcompgm
    txt_damcompgm = StringVar()
    global txt_excludepaths
    txt_excludepaths = StringVar()
    global txt_onlypaths
    txt_onlypaths = StringVar()
    global ckb_event
    ckb_event = StringVar()
    global ckb_gps
    ckb_gps = StringVar()
    global ckb_collections
    ckb_collections = StringVar()
    global ckb_people
    ckb_people = StringVar()
    global ckb_keywords
    ckb_keywords = StringVar()
    global ckb_categories
    ckb_categories = StringVar()
    global ckb_place
    ckb_place = StringVar()
    global ckb_title
    ckb_title = StringVar()
    global ckb_description
    ckb_description = StringVar()
    global ckb_comments
    ckb_comments = StringVar()
    global ckb_verify_basenames
    ckb_verify_basenames = StringVar()
    global rbtn_verbose_gui
    rbtn_verbose_gui = StringVar()
    global Recent1
    Recent1 = StringVar()
    global recent2
    recent2 = StringVar()
    global rbtn_analysis_type
    rbtn_analysis_type = StringVar()
    global msg_error
    msg_error = StringVar()
    ...
    alltags = ["Event", "Place", "GPS", "Title", "Description", "Comments", "People", "Keywords", "Categories", "Collections"]
...
def populate_GUI_panel():  # Set Values to display at the GUI
    ...
    # -t [{Event,Place,GPS,People,Keywords,Categories} [{Event,Place,GPS,People,Keywords,Categories} ...]],
    # --tags [{Event,Place,GPS,People,Keywords,Categories} [{Event,Place,GPS,People,Keywords,Categories} ...]]

    # Define one checkbox for each tag in list alltags
    ckb_event.set(False);
    ckb_place.set(False);
    ckb_gps.set(False);
    ckb_people.set(False);
    ckb_keywords.set(False)
    ckb_categories.set(False);
    ckb_collections.set(False);
    ckb_title.set(False);
    ckb_comments.set(False)
    ckb_description.set(False)
    ...
    if args.exdir is not None:
        if args.exdir != []:
            txt_excludepaths.set('"' + '" "'.join(map(str, args.exdir)) + '"')
    ...
    if args.outfilename is not None:  # -o OUTFILE, --output OUTFILE Output file for report [stdout]
        if args.outfilename not in ("<stdout>", "", " "):
            ...
            ckb_display_only.set("0")
        else:
            ckb_display_only.set("1")
    if int(rbtn_verbose_gui.get()) > 1:
...
def WriteIni(damscaninifile):
    ...
    elif ckb_verify_basenames.get() == '1':
    ...
    with open(damscaninifile, "w") as configfile:
        ...
        if rbtn_verbose_gui.get() == '1':
            verbose_gui = 1
        elif rbtn_verbose_gui.get() == '2':
            verbose_gui = 2
        else:
            verbose_gui = 0
        if verbose_gui > 0: print('DamScanGUI - Config file "' + os.path.abspath(configfile.name), end='" written')
    return configfile
...
def read_config_for_gui(newdamscanini):
    ...
    # print('Function read_config_for_gui line:', getframeinfo(currentframe()).lineno,
    #       "\nargs:", args, "\nnewdamscanini:", newdamscanini, flush=YES)                         # for debugging
    read_ini(args, conf)
    ...
    #    print("testforinifile.name:", newdamscanini)

    #    args.ini_file=newdamscanini.replace(".ini","")
    ...
    # print('Function read_config_for_gui:', getframeinfo(currentframe()).lineno,    # For Debbugging
    #       "args.ini_file :",args.ini_file,"newdamscanini: ", newdamscanini, "\n args:",args, "\ntxt_newdamscanini.get()",txt_newdamscanini.get(), "\ntxt_damscanini.get()",txt_damscanini.get())
...
def browse_dmc_1(event=None):  # see https://pythonspot.com/en/tk-file-dialogs/
    dmcfile = filedialog.askopenfilename(initialdir=txt_dbpath.get(),
                                         title="Select a Local Daminion Catalog",
    ...
    if dmcfile != "":
        ...
        txt_catalog_dates.set(filecreatedate + ", " + filechangedate)
        ckb_sqlite.set("1")  # If we find the file locally, we assume SQLite
    return
    ...
    if ckb_sqlite.get() == "1":
        ...
        try:
            ...
            if not server_attributes_changed:           # if none of the server attributes were changed in the panel,
                if not args.sqlite: ckb_sqlite.set("0") # we ignore errors accessing the server and do not change
            ...
            if args.sqlite: ckb_sqlite.set("1")
            return
...
def build_CMD_line():
    ...
    args.taglist = []
    if ckb_event.get() == '1':      args.taglist.append("Event")
    if ckb_place.get() == '1':      args.taglist.append("Place")
    if ckb_gps.get() == '1':        args.taglist.append("GPS")
    if ckb_people.get() == '1':     args.taglist.append("People")
    if ckb_keywords.get() == '1':   args.taglist.append("Keywords")
    if ckb_categories.get() == '1': args.taglist.append("Categories")
    if ckb_collections.get() == '1': args.taglist.append("Collections")
    if ckb_title.get() == '1': args.taglist.append("Title")
    if ckb_comments.get() == '1':    args.taglist.append("Comments")
    if ckb_description.get() == '1': args.taglist.append("Description")
    ...
    if analysis_type == "Scan":
        ...
        if txt_basename.get() != "":  # check base file name
            ...
            ckb_verify_basenames.set("1")
        elif ckb_verify_basenames.get() == "1":  # check only file extensions
            args.basename = " "
            newArgString += " -b "
...
def select_ini_file(event=None):
    global RecentDamScanINIfiles, w
    ...
    if int(rbtn_verbose_gui.get()) > 1:
        print('Func. select_ini_file():', getframeinfo(currentframe()).lineno, event,
                                              w.Recent_ini_files1.curselection(), "\n")
    if i >= 0:
        print("Options file:", RecentDamScanINIfiles[i], "selected.")
